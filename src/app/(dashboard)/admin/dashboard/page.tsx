...
export default async function DashboardPage() {
  ...
  // Fetch counts conditionally depending on role / permissions to optimize queries
  const isSuperAdmin = admin.role === 'superadmin';
  const hasMediaPermission = isSuperAdmin || admin.permissions?.includes('media_management');
  const hasBookingsPermission = isSuperAdmin || admin.permissions?.includes('bookings_management');

  const promises: Promise<number>[] = [
    isSuperAdmin ? AdminUser.countDocuments() : Promise.resolve(0),
    hasMediaPermission ? Media.countDocuments() : Promise.resolve(0),
    hasBookingsPermission ? Booking.countDocuments() : Promise.resolve(0),
  ];

  const [adminCount, mediaCount, bookingsCount] = await Promise.all(promises);
  ...
  // Set grid columns based on stats count
    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-1 text-sm sm:text-base">Welcome back, {admin.name}! Real-time overview of your platform.</p>
        </div>
